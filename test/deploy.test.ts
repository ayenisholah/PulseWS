...
describe("production container and Compose cluster", () => {
  ...
  test("provides an automatic stop-on-first-failure measured capacity benchmark", async () => {
    ...
    expect(runner).toContain("host-network-limits.txt");
    expect(runner).toContain("container-limits.txt");
    ...
    expect(runner).toContain("redis_reservations");
    expect(runner).toContain("nginx.log");
    ...
    expect(scenario).toContain("summaryTrendStats");
    expect(scenario).toContain("pulsews_capacity_publish_4xx");
    expect(scenario).toContain("pulsews_capacity_publish_5xx");
    expect(scenario).toContain("pulsews_capacity_upgrade_4xx");
    expect(scenario).toContain('pulsews_capacity_publish_failures{status:503}');
